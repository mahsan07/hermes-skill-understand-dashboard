...
This package expresses **Understand Dashboard** as a typed, task-adaptive agent harness. Its concrete capability remains: Launch an interactive dashboard that visualizes a codebase knowledge graph. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Understand Dashboard |
| --- | --- | --- |
| Memory | `resource_state_registry` | Retain code knowledge graph plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `resource_orchestration_plan` | Transform graph service into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through filters and search while preserving stop conditions and user authority. |
